...
export function OrganizerEventsSection({
  ...
  const submitEdit = async (ev: FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`${apiBase}/api/events/${editing.id}`, {
        method: "PUT",
  ...
  const toggleRegistrations = async (e: OrganizerEventItem, open: boolean) => {
    setActionBusyId(e.id);
    try {
      const path = open ? "open" : "close";
      const response = await fetch(`${apiBase}/api/events/${e.id}/registrations/${path}`, {
        method: "POST",
  ...
  const loadFeedback = async (e: OrganizerEventItem) => {
    setFeedbackFor(e);
    setFeedbackRows([]);
    setFeedbackLoading(true);
    try {
      const response = await fetch(`${apiBase}/api/feedback/event/${e.id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (response.ok) setFeedbackRows((await response.json()) as FeedbackApiRow[]);
    } finally {
      ...
      {editing ? (
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/40 p-4 backdrop-blur-sm">
          <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl border border-slate-200/80 bg-white shadow-2xl">
            ...
            <form onSubmit={(ev) => void submitEdit(ev)} className="grid gap-3 bg-slate-50/60 p-6 md:grid-cols-2">
              <input
